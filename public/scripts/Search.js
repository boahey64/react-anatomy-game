document.addEventListener('DOMContentLoaded', () => {
  function filterQuizzes(keyword) {
    ...
    allQuizPreviews.forEach((quizPreview) => {
      const keywords = quizPreview
        .querySelector('.quiz-picture')
        .getAttribute('data-keywords')
        .toLowerCase()
        .split(',');
      const quizInfo = quizPreview
        .querySelector('.quiz-info')
        .textContent.toLowerCase();

      if (
        keywords.some((kw) => kw.trim() === keyword.toLowerCase()) ||
        quizInfo.includes(keyword.toLowerCase())
      ) {
